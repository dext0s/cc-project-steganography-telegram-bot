...
class TelegramBot(object):
    ...
    def __init__(self, token, tmp_dir):
        ...
        dispatcher.add_handler(MessageHandler(Filters.document & Filters.caption,self.encryptLSBImgText))
        dispatcher.add_handler(MessageHandler(Filters.document,self.decryptLSBImg2Text))
    ...
    def encryptLSBImgText(self ,update: Update, context: CallbackContext) -> None:
        ...
        encrypt = SteganographyImg(img_path,out_path).encryptLSBImgText(msg_to_encrypt)
        if encrypt :
            out_img = open(out_path, 'rb')
            context.bot.send_document(chat_id=update.effective_chat.id, document=out_img)
            out_img.close()
        pass
    ...
    def decryptLSBImg2Text(self, update: Update, context: CallbackContext) -> None:
        ...
        img_path=self.__get_image(update)
        decrypted_msg= SteganographyImg(img_path,img_path).decryptLSBImg2Text()
        
        context.bot.send_message(chat_id=update.effective_chat.id, text=decrypted_msg)
        pass
